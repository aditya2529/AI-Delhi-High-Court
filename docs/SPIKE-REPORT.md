...
| 2026-05-17 | W.P.(C) 6569/2023 (aircraft leasing — has judgment PDF) | **Pipeline FAILED — HTTP 500** | Founder report: opaque 500 with `request_id=b1704a5c3c7741b9bbbac334b8ba4083`. Three `search.submit.no_audit_row` warnings observed at `search_service.py:270` across this + the next two rows. Root cause + fix: see Section I below. |
| 2026-05-17 | W.P.(C) 10327/2023 (connected) | **Pipeline FAILED — HTTP 500** | Founder report. Same failure pattern. |
| 2026-05-17 | W.P.(C) 6626/2023 (connected) | **Pipeline FAILED — HTTP 500** | Founder report. Same failure pattern. Founder's qualitative read: "system always asking me for the second time" (consistent with the captcha-retry path identified in Section I). |
...
## Section I — 2026-05-17 incident: three /2023 cases returned HTTP 500

> Diagnosis by Arjun (backend lead). Founder ran 4 `CLIENT_MODE=real`
> cases. 1/1 (`W.P.(C) 2344/2024`) succeeded; 0/3 (`6569/2023`,
> `10327/2023`, `6626/2023`) failed with opaque HTTP 500. The failing
> `request_id=b1704a5c3c7741b9bbbac334b8ba4083` was generated by the
> global `_unhandled_handler` in `backend/app/main.py` (32-char
> `uuid4().hex` matches the mint pattern).

### Two independent bugs were collapsing into one user-facing 500

**Bug 1 — Upstream 5xx HTML leaked into the parser**

`DelhiHCClient._request` raised `CourtClientError` on 4xx responses
(`raise_on_4xx=True`) but did NOT raise on 5xx — it returned the
response object instead. `submit_search` then handed the 500 HTML
body to `DHCParserV1.parse_with_outcome`. If the upstream 500 page
didn't match any sentinel selector, the parser tried `_extract_case`
on a non-case page; uncaught exceptions there fell through to the
global handler → HTTP 500 with a fresh `request_id`.

Fix: `_request` now raises `CourtClientError(... returned 5xx after
retry)` when the retried response is still 5xx and the caller wants
4xx-style mapping. The submit route then maps to
`status='court_error'` per API-CONTRACT §3.

Pinned by `TestSubmit5xxMapping` in
`backend/tests/unit/test_delhi_hc_client_contract.py`.

**Bug 2 — Retry on the same session 404'd because no eligible audit row**

`_latest_search_request_for` filters audit rows by
`status IN ('initialized','captcha_displayed')`. After a first
`/submit` (regardless of outcome) the row is flipped to `submitted`,
`success`, or `failed`. The session itself, however, is NOT deleted
on `captcha_failed` or `court_error` — the user can retry. The
retry then finds no eligible audit row, logs the warning
`search.submit.no_audit_row`, returns `None`, and the route raises
`session_not_found` (404). That mismatch ("session is alive but I
get told it's not found") is consistent with the founder's
qualitative report.

Fix: when no eligible row exists but the session is alive, mint a
follow-on audit row inline and proceed. The
`search.submit.no_audit_row` warning still fires but now carries
`outcome=minted_followon_audit_row` so it's visible without breaking
the user flow.

Pinned by `TestRetryOnSameSession::test_wrong_then_right_captcha_succeeds`
in `backend/tests/integration/test_search_flow.py`.

### /2023 vs /2024 — coincidence

n=1 success (`/2024`) vs n=0 success (3 × `/2023`) is not a sample
big enough to support a /2023-vs-/2024 causal claim. The Delhi HC
form does not route differently on the year (same endpoint, same
field name `case_year`, identical 4-digit format). The most likely
explanation is upstream-side flakiness on three sequential submits
in quick succession — possibly a rate-limit-adjacent 5xx behavior
the spike's polite-pacing recon did not exercise (B.7 partial,
submit-endpoint threshold still unknown).

**Recommendation:** treat the /2023 vs /2024 angle as unconfirmed
until the founder captures ≥5 more `/2023` and ≥5 more `/2024`
cases under the same pacing. With the two fixes above and the new
structured logging (`dhc.step` event in
`backend/app/clients/delhi_hc_client.py`), the next failed run will
have a grep-able trail.

### request_id b1704a5c... — likely traceback

Without file logging (DX agent's bucket — `logs/backend/app.log`
was 0 bytes on the incident day) the literal stack frame cannot be
recovered. The shape that fits all the observed evidence is:

```
File "backend/app/services/search_service.py", line 283, in submit_search
  return await _parse_and_cache(...)
File "backend/app/services/search_service.py", line 342, in _parse_and_cache
  outcome: ParseOutcome = parser.parse_with_outcome(
File "backend/app/parsers/case_parser.py", line ~226, in parse_with_outcome
  case = self._extract_case(soup, ...)  # raises on 500-page HTML
<...AttributeError or KeyError in extraction against non-case markup...>
```

The hypothesis is consistent with Bug 1: a 500 page reached the
parser with no sentinel match. Once Bug 1's fix is in (5xx →
`CourtClientError`), this path is unreachable — the route returns
`status='court_error'` (HTTP 200) instead.

### Structured logging added

`init`, `validate`, and `submit` now emit a `dhc.step` event each
with: `agent`, `case_type`, `case_number`, `year`, `http_status`,
`elapsed_ms`, `cookie_names` (names only, never values), `outcome`
(`success|captcha_failed|http_error|exception`), and `error` (where
applicable). `structlog`'s `merge_contextvars` processor already
splices `request_id` onto every record from the route-layer
middleware, so a single grep for the request_id returns the full
init → validate → submit trail.

### What the founder needs next sprint

1. **Land the DX file-logging fix** (`TestFileLoggingDoD` failures in
   `test_admin.py`). Without it, the structured trail goes to
   stderr and not to disk, so a mystery `request_id` from a
   post-shutdown report is still un-greppable.
2. **Capture more real-mode runs** (≥5 × /2023, ≥5 × /2024) to
   either confirm or kill the "year matters" theory.
3. **Probe the submit-endpoint rate limit** (B.7 residual) — three
   sequential submits in <1 minute is the most likely trigger of
   the upstream 5xx.
4. **Defer the `_pending` dict concurrency fix** to the planned
   sprint with Raj's ABC change (`DelhiHCClient._pending`
   sequential-single-user constraint). Not implicated in this
   incident (3 sequential submits, no concurrency), but is the
   right place for the next round of robustness work.
